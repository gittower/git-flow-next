...
// TestFinishAbortAfterManualResolve reproduces gittower/git-flow-next#110: a
// finish hits a merge conflict, the user resolves it manually and commits
// (which removes MERGE_HEAD without going through `finish --continue`), leaving
// merge.json stale. A later `finish --abort` must still exit 0 and clear the
// stale state so the repository is unblocked for the next finish.
func TestFinishAbortAfterManualResolve(t *testing.T) {
	...
	if _, err = testutil.LoadMergeState(t, dir); err != nil {
		t.Fatalf("Expected merge.json to still exist before abort, got error: %v", err)
	}

	// `finish --abort` must succeed even though git has nothing to abort.
